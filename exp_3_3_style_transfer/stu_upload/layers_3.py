...
class StyleLossLayer(object):
    ...
    def forward(self, input_layer, style_layer):
        ...
        self.gram_style = np.zeros([style_layer.shape[0], style_layer.shape[1], style_layer.shape[1]]) # check the formula of Gram, whose shape is [N, C, C]
    ...
    def backward(self, input_layer, style_layer):
        ...
        for idxn in range(input_layer.shape[0]):
            # TODO： 计算风格损失的反向传播
            bottom_diff[idxn, :, :] = np.matmul((self.gram_input - self.gram_style)[idxn, :, :].T, style_layer.reshape(style_layer.shape[0], style_layer.shape[1], -1)[idxn, :, :]) \
