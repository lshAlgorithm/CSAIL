...
class T2ISampler(BaseSampler):
    ...
    def preprocess(
        ...
        prior_cf_scales_batch = torch.tensor(prior_cf_scales_batch, device="mlu")

        decoder_cf_scales_batch = [self._decoder_cf_scale] * len(prompts_batch)
        decoder_cf_scales_batch = torch.tensor(decoder_cf_scales_batch, device="mlu")
        ...
        tok, mask = tok.to(device="mlu"), mask.to(device="mlu")
